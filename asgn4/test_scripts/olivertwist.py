...
def create(rid, event, hostname, port, outdir):
    ...
    block = 0
    if "block" in event and event["block"] >= 1:
        block = event["block"]

    return request(hostname, port, method, uri, rid, inbody, outfile, block)
...
#            requests[rid].sock.shutdown(socket.SHUT_WR)
            ...
            fd = req.sock.fileno()
            if fd > 0:
                if not req.state == Req_State.RECEIVED:
                    if not requests[rid].block:
